...
bool CoreChecks::ValidatePipelineDrawtimeState(LAST_BOUND_STATE const &state, const CMD_BUFFER_STATE *pCB, CMD_TYPE cmd_type,
    ...
    if (pPipeline->vertex_binding_descriptions_.size() > 0) {
        for (size_t i = 0; i < pPipeline->vertex_binding_descriptions_.size(); i++) {
            ...
            if ((pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings.size() < (vertex_binding + 1)) ||
                (pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings[vertex_binding].buffer == VK_NULL_HANDLE)) {
                skip |=
        ...
        for (size_t i = 0; i < pPipeline->vertex_attribute_descriptions_.size(); i++) {
            ...
            if ((vertex_binding_map_it != pPipeline->vertex_binding_to_index_map_.cend()) &&
                (vertex_binding < pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings.size()) &&
                (pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings[vertex_binding].buffer != VK_NULL_HANDLE)) {
                ...
                const auto vertex_buffer_offset =
                    pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings[vertex_binding].offset;
                const auto buffer_state =
                    GetBufferState(pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings[vertex_binding].buffer);
                ...
                if (SafeModulo(attrib_address, vtx_attrib_req_alignment) != 0) {
                    skip |= log_msg(
                        ...
                        HandleToUint64(pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings[vertex_binding].buffer),
                        ...
                        report_data
                            ->FormatHandle(
                                pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings[vertex_binding].buffer)
                            .c_str());
                }
            }
        }
    } else {
        if ((!pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings.empty()) && (!pCB->vertex_buffer_used)) {
            skip |=
...
void CoreChecks::ResetCommandBufferState(const VkCommandBuffer cb) {
    ...
    if (pCB) {
        ...
        pCB->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings.clear();
...
void CoreChecks::PreCallRecordCmdBindIndexBuffer(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset,
    ...
    cb_state->object_bindings.emplace(buffer, kVulkanObjectTypeBuffer);
}
...
void CoreChecks::PreCallRecordCmdBindVertexBuffers(VkCommandBuffer commandBuffer, uint32_t firstBinding, uint32_t bindingCount,
    ...
    if (cb_state->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings.size() < end) {
        cb_state->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings.resize(end);
    }

    for (uint32_t i = 0; i < bindingCount; ++i) {
        auto &vertex_buffer_binding = cb_state->current_bind_vertex_buffer_binding_info.vertex_buffer_bindings[i + firstBinding];
        ...
        cb_state->object_bindings.emplace(pBuffers[i], kVulkanObjectTypeBuffer);
        auto buffer_state = GetBufferState(pBuffers[i]);
        buffer_state->cb_bindings.insert(cb_state);
    }
}
